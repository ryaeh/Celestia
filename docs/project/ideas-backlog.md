...
| **Access-based ranking** | Medium / Medium | Track `last_recalled` + `recall_count`; rank stale-never-recalled items down in retrieval (never delete — rank-only, consistent with Feature 10). Keeps injection quality high as the store grows. |
...
| Idea | Value/Effort | Notes |
|------|--------------|-------|
| **"Tidying while you're away"** | High / Medium | When the PC is idle and the GPU is free (`gpu.py` knows), run hygiene jobs: deep graph extraction, dedupe, re-ranking, session titling. She wakes up sharper; chat-time latency never pays for it. The natural consumer of the residency manager. |
